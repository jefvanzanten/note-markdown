<script lang="ts">
  ...
  const isHorizontalRuleLine = (lineText: string): boolean =>
    horizontalRulePattern.test(lineText);
    ...
    class {
      ...
      build(view: EditorView): DecorationSet {
        ...
        for (let ln = 1; ln <= view.state.doc.lines; ln++) {
          ...
          const horizontalRule = isHorizontalRuleLine(line.text);

          if (horizontalRule && ln !== activeLine) {
